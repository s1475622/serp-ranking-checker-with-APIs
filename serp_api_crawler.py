...
class SERPApiCrawler:
    def __init__(self, api_key=None):
        print("[INFO] 初始化SERP API爬蟲...")
        # 如果提供了單一 API key，將其轉為列表
        if api_key:
            self.api_keys = [api_key]
        # 否則從環境變數或配置文件獲取
        elif os.environ.get("SERPAPI_KEY"):
            self.api_keys = [os.environ.get("SERPAPI_KEY")]
        else:
            # 從配置文件讀取多組 API keys
            self.api_keys = config.SERPAPI_KEYS if hasattr(config, 'SERPAPI_KEYS') else [config.SERPAPI_KEY]

        # 過濾掉空值
        self.api_keys = [key for key in self.api_keys if key]

        if not self.api_keys:
            print("[警告] 未設置SERPAPI_KEY！請在config.py中設置或使用--api_key參數提供")
            print("您可以在 https://serpapi.com 註冊獲取免費API密鑰")
            print("[提示] 試用版API密鑰可提供每月100次免費查詢")
        else:
            print(f"[INFO] 已載入 {len(self.api_keys)} 組 API keys")
    ...
    def search_keyword(self, keyword, domain, region='US', max_results=100):
        """使用SERP API搜索關鍵字並找到網域排名（支援分頁以獲取100個結果）"""
        try:
            ...
            while len(all_organic_results) < max_results and not found_target:
                ...
                if language == "en":
                    ...
                    params["lr"] = "lang_zh-TW"

                print(f"[INFO] 發送API請求（第 {page} 頁，起始位置: {current_start}）...")
                ...
                if response.status_code != 200:
                    ...
                    if response.status_code == 429 or "rate limit" in response.text.lower() or "quota" in response.text.lower():
                        ...
                        if self._switch_to_next_api_key():
                            print(f"[INFO] 已切換到第 {self.current_api_index + 1} 組 API key，重新嘗試...")
                            # 遞迴重試（使用新的 API key）
                            return self.search_keyword(keyword, domain, region, max_results)
                        else:
                            print(f"[錯誤] 所有 API keys 都已達到配額限制")
                            return {
                                'keyword': keyword,
                                'domain': domain,
                                'position': -2,
    ...
    def _switch_to_next_api_key(self):
        """切換到下一組 API key，如果還有可用的話"""
        if self.current_api_index < len(self.api_keys) - 1:
            self.current_api_index += 1
            self.api_key = self.api_keys[self.current_api_index]
            print(f"[INFO] 切換 API key: {self.current_api_index + 1}/{len(self.api_keys)}")
            return True
        return False
        ...
        try:
            ...
            for index, row in df.iterrows():
                ...
                for region in config.SELECTED_REGIONS:
                    ...
                    # 檢查是否達到API配額限制（所有 API keys 都用完）
                    if result.get('position') == -2 and result.get('error') and '所有API配額都已達限制' in result.get('error'):
                        print("\n" + "="*60)
                        print("[警告] 所有 API 配額都已達限制，停止爬取並保存已完成的結果")
                        print("="*60 + "\n")
                    ...
                    if index < len(df) - 1 or region != config.SELECTED_REGIONS[-1]:
                        delay = config.API_DELAY_BETWEEN_REQUESTS
            ...
            output_file = os.path.join(
                ...
                f'serp_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.{config.OUTPUT_FILE_FORMAT}'
            )
...
def main():
    try:
        print("[INFO] 啟動程序...")
        parser = argparse.ArgumentParser(description='SERP API排名追蹤器')
        ...
        parser.add_argument('--keywords_file', type=str, default='keywords.csv',
                          help='關鍵字文件路徑')
        parser.add_argument('--api_key', type=str, help='SERP API 密鑰')
        ...
        print("[INFO] 初始化SERP API爬蟲...")
        crawler = SERPApiCrawler(api_key=args.api_key)
        try:
            print("[INFO] 開始處理關鍵字...")
